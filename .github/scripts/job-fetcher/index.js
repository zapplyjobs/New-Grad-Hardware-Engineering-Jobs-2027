...
/**
 * Main entry point for the job fetcher system - New-Grad-Hardware-Engineering-Jobs-2026
 *
 * Fetches hardware engineering jobs from centralized aggregator and generates README.
 * Replaced JSearch direct fetch with aggregator consumer (2026-02-18).
 *
 * Pipeline:
 * 1. Fetch hardware engineering jobs from aggregator (jobs-data-2026/all_jobs.json)
...
async function main() {
  try {
    console.log('🚀 Starting Hardware Engineering jobs fetching system...');
    ...
    if (jobs.length === 0) {
      console.log('⚠️  No hardware engineering jobs fetched from aggregator');
      console.log('   Check that all_jobs.json exists in jobs-data-2026');
    }

    console.log(`\n✅ Fetched ${jobs.length} hardware engineering jobs from aggregator`);
